...
kf <- kf %>%
  group_by(`two digit`) %>%
    summarize(rmmean = weighted.mean(spitz_r_manual, ct2))
kf
```
...
```{r}
ggplot(df, aes(reorder(occupation.x, rmmean), y=log_distance_change, color=`occupation.y`)) + geom_point(size=5)+  
  theme_light() +
  ...
  labs(title="Change in Distance (pca3), sorted by occupational routine manual skill", x="Occupation, sorted by 1980 weighted routinue manual skill (asc)", y="Change in distance from 1980-1990(higher-more dissimilarity)")
